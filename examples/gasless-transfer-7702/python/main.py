"""ZeroDev Gasless Transfer via EIP-7702 — Python Example.
...
the SDK signs an authorization tuple `(chainId, kernelImpl, nonce)` and
attaches it via the `eip7702Auth` field; subsequent ops skip the auth once the
delegation is installed on-chain.
...
def main() -> None:
    print("=======================================================")
    print("  ZeroDev Gasless Transfer (EIP-7702) — Python Example")
    print("=======================================================")
    print()
    ...
    with Context(project_id, chain_id=11155111) as ctx:
        ...
        # ── Step 3: Create signer — fresh random EOA by default ────────
        if pk_hex:
            ...
            print("[3/6] Signer created (fresh random EOA — no prior delegation)")

        with signer:
            # ── Step 4: Create EIP-7702 delegated account ──────────────
            # The account address is the signer's EOA address; no CREATE2, no
            # init code, no index. Today only Kernel v3.3 supports EIP-7702.
            with ctx.new_account_7702(signer, KernelVersion.V3_3) as account:
                addr = account.get_address()
                print(f"[4/6] EIP-7702 account ready: {addr.hex()}")
                print("       (EOA == smart-account address)")

                # ── Step 5: Build a call (send 0 ETH to self) ──────────
                call = Call(target=addr.bytes)  # 0 ETH, empty calldata
                print("[5/6] Sending sponsored EIP-7702 UserOp...")
                print("       (SDK signs and attaches the auth tuple on first op)")
                ...
                # ── Wait for receipt ───────────────────────────────────
                print("[6/6] Waiting for receipt (timeout: 90s, poll: 2s)...")
                receipt = account.wait_for_receipt(userop_hash, timeout_ms=90000, poll_ms=2000)
